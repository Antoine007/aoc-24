...
def parse_input(input):
    ...
    seed_intervals = [
        Interval(seeds[i], seeds[i + 1]) for i in range(0, len(seeds), 2)
    ]
    ...
    for x in range(7):
        _, line = next(lines), next(
            lines)  #Skip the blank line and header lines
...
def part2(seed_intervals: IntervalList, interval_lists: List[IntervalList]):
    ...
    for intervalList in interval_lists[::
                                       -1]:  # Go in reverse order from the last map to the first map
    ...
    candidate_points = seed_intervals.filter(
        candidate_points)  # Remove candidates that are not valid

    return part1(
        candidate_points,
        interval_lists)  # Test the candidates and find the optimal one
